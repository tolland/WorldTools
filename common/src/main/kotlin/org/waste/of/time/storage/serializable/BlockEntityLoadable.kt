...
class BlockEntityLoadable(
    ...
    fun load(
        ...
        val savedEntitiesAll = storage.getBlockEntities(chunkPos)

        LOG.info("[WT-merge] chunk $chunkPos: ${savedEntitiesAll.size} saved block entities, ${cachedBlockEntities.size} in memory")

        val savedEntities = savedEntitiesAll.filter { it.isSupported }
    ...
    private fun LockableContainerBlockEntity.migrateData(existing: BlockEntity) {
        ...
        LOG.info("[WT-merge]  $pos: live container found, checking if it can be merged with saved data...")
